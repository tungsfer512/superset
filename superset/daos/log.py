...
class LogDAO(BaseDAO[Log]):
    ...
    def get_recent_activity(
        ...
        # `time_delta_humanized` is generated by `humanize.naturaltime()` (backend),
        # not by frontend i18n `t()/tn()`. Activate humanize's own locale based on
        # the current Flask-Babel locale so phrases like "minutes ago" can be localized.
        try:
            locale = get_locale()
        except Exception:
            # If there's no request/app context, fall back to default (English).
            locale = None

        if locale:
            # humanize typically expects locale format like `vi_VN`
            normalized_locale = str(locale).replace("-", "_")
            try:
                humanize.i18n.activate(normalized_locale)
            except FileNotFoundError:
                # Some libs ship Vietnamese as `vi_VN` instead of `vi`.
                if normalized_locale.lower() == "vi":
                    try:
                        humanize.i18n.activate("vi_VN")
                    except FileNotFoundError:
                        humanize.i18n.deactivate()
                else:
                    # humanize doesn't have this locale installed; keep default behavior.
                    humanize.i18n.deactivate()
        else:
            humanize.i18n.deactivate()
        ...
        try:
            ...
            # Reset locale after use to avoid leaking locale settings across requests/threads.
            humanize.i18n.deactivate()
