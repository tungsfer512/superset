...
PUBLIC_ROLE_LIKE: str | None = os.getenv("PUBLIC_ROLE_LIKE", "Gamma")
...
# each cache config.
CACHE_DEFAULT_TIMEOUT = int(timedelta(days=5).total_seconds())
...
DATA_CACHE_CONFIG: CacheConfig = CACHE_CONFIG.copy()
...
if os.environ.get("SUPERSET_LOG_LEVEL"):
    LOG_LEVEL = getattr(logging, os.environ["SUPERSET_LOG_LEVEL"].upper(), LOG_LEVEL)
...
if os.environ.get("SUPERSET_LOG_LEVEL"):
    TIME_ROTATE_LOG_LEVEL = getattr(
        logging, os.environ["SUPERSET_LOG_LEVEL"].upper(), TIME_ROTATE_LOG_LEVEL
    )
...
RESULTS_BACKEND: BaseCache | None = FileSystemCache("/app/superset_home/sqllab")
